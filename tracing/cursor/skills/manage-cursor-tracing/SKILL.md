...
The config file at `~/.arize/harness/config.json` is the single source of truth for backend credentials and per-harness settings. Create the directory structure if needed: `mkdir -p ~/.arize/harness/{bin,run,logs,state/cursor}`
...
- **`sessionEnd`** produces a `Session End` CHAIN span with `cursor.session.duration_ms`, `cursor.session.final_status`, and `cursor.session.reason`.
- **`stop`** produces an `Agent Stop` CHAIN span carrying per-turn status / loop_count / duration metadata.
- **Terminal lifecycle and token attribution.** `stop` and `sessionEnd` are distinct once-only terminal events that can arrive in either order (real hosts differ: the IDE emits `stop`, the CLI emits `sessionEnd`). Whichever terminal event arrives **first** completes any deferred `afterAgentResponse` LLM span and attaches the terminal payload's token usage (`llm.token_count.prompt`, `llm.token_count.completion`, cache-read/write subsets, total, and model name) to it; without a deferred response, the counts land on that first terminal CHAIN span instead. Cumulative token counts are attributed exactly once per generation — a later terminal event never re-attaches them. Both terminal spans parent under the generation root; this is event-driven rather than inferred from IDE/CLI payload casing.
