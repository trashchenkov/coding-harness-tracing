...
def _install_hooks() -> None:
    """Write/merge our hook entries into ~/.qwen/settings.json.

    Every event in EVENTS is registered, including ones added after 0.15.11.
    Qwen Code ignores hook entries for event names it does not know, so an older
    CLI simply never fires them rather than failing to start.
    """
    if dry_run():
    ...
    for event, entry_point in EVENTS.items():
        ...
        # Append our block. An empty matcher matches every trigger; events that
        # do not support matchers ignore the field.
        event_list.append(
            {
                "matcher": "",
                "hooks": [
